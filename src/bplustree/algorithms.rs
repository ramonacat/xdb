...
fn insert_split_leaf<TStorage: Storage, TKey: Pod + Ord>(
    ...
    mut new_node: LeafNode<TKey>,
    root_index: AnyNodeId,
) -> Result<(), TreeError> {
        ...
        match split_node {
            Some((new_leaf, mut new_interior)) => {
                ...
                transaction.write_node(grandparent_id, |node| {
                    let grandparent_insert_result = node
                        .insert_node(&new_interior.first_key().unwrap(), new_interior_id.into());
                ...
                transaction
                    .insert_reserved(new_interior_reservation, Page::from_data(*new_interior))?;
            }
            None => {
